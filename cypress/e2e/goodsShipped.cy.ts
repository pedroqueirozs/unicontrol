...
describe("Testes de mercadorias enviadas", () => {
  ...
  it("Deve mostrar erros nos campos obrigatórios ao tentar salvar sem preencher", () => {
    ...
    cy.get("span.text-notification_error").should("exist");
  });
  ...
  it("Deve mostrar erro quando previsão de entrega for anterior à data de envio", () => {
    ...
    cy.contains("Salvar").click();
    cy.contains("Deve ser igual ou posterior à data de envio").should("exist");
  });
  ...
  it("Deve cadastrar uma mercadoria com sucesso", () => {
    cy.get('input[placeholder="Buscar por nome ou CNPJ/CPF..."]').type("a");
    ...
    cy.contains("Salvar").click();
    cy.contains("Tem certeza que deseja cadastrar essa mercadoria?", { timeout: 8000 }).should("be.visible");
