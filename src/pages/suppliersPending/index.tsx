...
const createSchema = yup.object({
  supplierName: yup.string().required("*").max(200, "Máximo 200 caracteres"),
...
export default function SuppliersPending() {
  ...
  const [data, setData] = useState<SupplierPendingUI[]>([]);
  ...
  // Modal de detalhe
  const [detailItem, setDetailItem] = useState<SupplierPendingUI | null>(null);
  ...
  async function loadData() {
    ...
    try {
      const q = query(
        collection(db, "companies", companyId, "suppliers_pending"),
        orderBy("createdAt", "desc")
      );
      const snapshot = await getDocs(q);
      const list: SupplierPendingUI[] = snapshot.docs.map((d) => {
        const raw = d.data() as SupplierPendingFirestore;
  ...
  async function handleAddUpdate() {
    ...
    try {
      ...
      await updateDoc(
        doc(db, "companies", companyId, "suppliers_pending", detailItem.id),
        { updates: arrayUnion(newUpdate) }
      );
      const updatedItem: SupplierPendingUI = {
        ...detailItem,
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await deleteDoc(
        doc(db, "companies", companyId, "suppliers_pending", id)
      );
  ...
  const columns: GridColDef[] = [
    ...
    { field: "supplierName", headerName: "Fornecedor", flex: 1, minWidth: 150 },
    ...
    <div>
      ...
      <div style={{ width: "100%" }}>
        <DataGrid
          ...
          onRowClick={(params) =>
            setDetailItem(params.row as SupplierPendingUI)
          }
          sx={{
            cursor: "pointer",
